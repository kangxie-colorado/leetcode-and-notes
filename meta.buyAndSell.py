...
class Solution:
    def maxProfit(self, prices: List[int]) -> int:
        """
        four states for each round
        i-1 round:  "owning 1 share" "sold 1 share" "owning 2nd share" "sold 2nd share"
        
        i round: ...

        "owning 1 share": my max profix at this round ending with state of owning 1 share.
        "sold 1 share": my max profit at this round, ending with state of 1 sold share
        ...

        state1 can go to
          state1, by doing nothing (for initial buy)
          state2
        stete2 can go to
          state2
          state3
        ...
        for i in range(1, len(prices)+1):
            dp[i][0] = dp[i-1][0]
            dp[i][1] = max(dp[i-1][1], dp[i-1][0]-prices[i-1]) # either owned it before or owned this round
            dp[i][2] = max(dp[i-1][2], dp[i-1][1] + prices[i-1]) # either sold it before or this round
            dp[i][2] = min(dp[i-1][3], dp[i-1][2]-prices[i-1]) # either bought it before and hold; or buy this round
            dp[i][3] = max(dp[i-1][4], dp[i-1][3] + prices[i-1]) # either sold it before; or sold this round

        return max(dp[len(prices)])    

"""
not sure, but the pattern doesn't work for me
okay.. going back to cost/profit 

treating first txn's profit as cost (negative)
"""
...
class Solution:
    def maxProfit(self, K: int, prices: List[int]) -> int:
        ...
        # at idx, remain k txn, the max profit
        @cache
        def f(idx, k):
            if idx>=N or k==0:
                return 0
            ...
            # buy or not buy
            # not buy at idx, profit will be
            no_buy_profit = f(idx+1, k)

            # buy at idx, where to sell
            buy_profit = 0
            for j in range(idx+1, N):
                buy_profit = max(buy_profit, prices[j] - prices[idx] + f(j+1, k-1))
    ...
    def maxProfit(self, K: int, prices: List[int]) -> int:
        ...
        for k in range(1, K+1):
            for idx in range(N-1,-1,-1):
                ...
                buy_profit = 0
                for j in range(idx+1, N):
                    buy_profit = max(buy_profit, prices[j] - prices[idx] + dp[k-1][j+1])
...
"""
okay.. still TLE
one extra loop is the reason

okay.. just apply the last round's profit (negative is my current cost)
"""
class Solution:
    def maxProfit(self, K: int, prices: List[int]) -> int:
        n = len(prices)
        ...
        for k in range(1,K+1):
            minCost = float('inf')
            for i in range(1, n):
                minCost = min(minCost, prices[i-1] - dp[k-1][i-1]) # selling at i-th day, cost is from i-1 day or before
