...
I did this before but with fraction number to break the tie
now I think I can just to a complete pre-order dump

fill the leafes with nill

so num[0] will be root
num[1] num[2] will be left right
num[3] num[4] will lbe num[1]'s left, right?

but the example 1's 4 and 5 will not conform to this
...
lets encode a tree like 
[root, [left], [right]]
this can be converted to str easily
>>> print(str([1,[2,3,[3.4,5]]]))
[1, [2, 3, [3.4, 5]]]

then I can parse this and restore 
>>> ast.literal_eval('[1, [2, 3, [3.4, 5]]]'.replace(" ",""))
[1, [2, 3, [3.4, 5]]]
"""
...
class Codec:
    ...
    def serialize(self, root):
        ...
        def helper(root):
            ...
            res = []
            res.append(root.val)
            res.append(helper(root.left))
            res.append(helper(root.right))
            

            return res

        return str(helper(root)).replace(" ", "")
    ...
    def deserialize(self, data):
            ...
            while i<len(data):
                ...
                if c == '[':
                    ...
                    while data[j].isdigit():
                        num = num*10 + int(data[j])
                        j+=1
                    i=j-1
                    curr.append(num*sign)
                i+=1
            return curr
        ...
        def helper(tree):
            ...
            root.left = helper(tree[1])
            print(tree)
            root.right = helper(tree[2])
            return root
...
okay it works but memory error here..

I think literal_eval has a limit?
maybe I can do that on my own

yeah.. that works..

Runtime: 272 ms, faster than 66.74% of Python3 online submissions for Serialize and Deserialize Binary Tree.
Memory Usage: 47.1 MB, less than 6.40% of Python3 online submissions for Serialize and Deserialize Binary Tree.

only use myEval but need to deal with '-'

Runtime: 164 ms, faster than 76.67% of Python3 online submissions for Serialize and Deserialize Binary Tree.
Memory Usage: 21.7 MB, less than 12.22% of Python3 online submissions for Serialize and Deserialize Binary Tree.
"""
...
def linklistTree(n):
    ...
    root = TreeNode(n)
    root.right = linklistTree(n+1)

    return root

"""
okay.. I should walk by a few more examples and see that nice structure
in which, once you mark the leaf or half leaf.. it will form a bfs structure 

the serialize works like level travesal.. why people keep saying preorder
it should be BFS

ah.. pre-order also works.. but that invovles queue and recursion.. kind of werid 
"""
...
"""
Runtime: 691 ms, faster than 5.03% of Python3 online submissions for Serialize and Deserialize Binary Tree.
Memory Usage: 20.4 MB, less than 45.63% of Python3 online submissions for Serialize and Deserialize Binary Tree.

also I can get rid of trailing 'n's to save some computation
Runtime: 723 ms, faster than 5.03% of Python3 online submissions for Serialize and Deserialize Binary Tree.
Memory Usage: 20.3 MB, less than 45.63% of Python3 online submissions for Serialize and Deserialize Binary Tree.

but not better
change to deque

Runtime: 631 ms, faster than 5.03% of Python3 online submissions for Serialize and Deserialize Binary Tree.
Memory Usage: 20.3 MB, less than 45.63% of Python3 online submissions for Serialize and Deserialize Binary Tree.

change serialize to deque as well
Runtime: 134 ms, faster than 81.40% of Python3 online submissions for Serialize and Deserialize Binary Tree.
Memory Usage: 20 MB, less than 92.96% of Python3 online submissions for Serialize and Deserialize Binary Tree.

okay.. this is good... and I didn't go thru the node
I go thru the queue when building the tree... 

now the pre-order 
ah. I see, the 'n' ensures a return to caller
single 'n' shift the left to right, or right to upper layer

using the example
            1 2 n n 3 4 n n 5 n n 
layer: 0      l
layer: 1 then first 'n' makes it to build right, and the next 'n' makes it to build layer-0's right
layer: 1            r so on and so forth
"""
...
"""
Runtime: 121 ms, faster than 90.47% of Python3 online submissions for Serialize and Deserialize Binary Tree.
Memory Usage: 20.5 MB, less than 31.96% of Python3 online submissions for Serialize and Deserialize Binary Tree.
"""
...
if __name__ == '__main__':
    ...
    # tool = Codec()
    # # data = tool.serialize(n1)
    # sys.setrecursionlimit(1500)

    # link1000 = linklistTree(1)
    # data = tool.serialize(link1000)

    # print(data)
    
    # tool.deserialize(data)
    ...
    data = tool.serialize(n1)
    print(data)
    data2 = tool.serialize(tool.deserialize(data))
    print(data2)